...
def cos_to_fit(file,in_dir,out_dir):
    ...
    img = ax0.pcolormesh(xmesh,ymesh,z_NS,cmap = cmap,norm = norm,shading='auto')
    ...
    img = ax1.pcolormesh(xmesh, ymesh, z_EW, cmap = cmap, norm = norm,shading='auto')
    ...
    hdu.writeto(outfile_dir + '/' + newfn + '.fits', overwrite=True)
    ...
    np.save(outfile_dir+'/'+filename+'.npy',pydata)
    #print()
